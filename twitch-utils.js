...
export async function getEnhancedItemTypeID(itemName) {
    ...
    if (isEveFilesTypeIDMapLoaded && eveFilesTypeIDMap.has(lowerCaseItemName)) {
        ...
        console.log(`[Twitch Utils] Found potential match in eve-files: "${itemName}" -> ${candidateTypeID}`);

        // Validate this is a published, tradeable item
        if (await isValidTradeableItem(candidateTypeID)) {
            console.log(`[Twitch Utils] Validated tradeable item: "${itemName}" -> ${candidateTypeID}`);
            typeIDCache.set(lowerCaseItemName, candidateTypeID);
            return candidateTypeID;
        } else {
            console.log(`[Twitch Utils] Item ${candidateTypeID} not tradeable, trying Fuzzwork instead`);
        }
    }
